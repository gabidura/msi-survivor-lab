#!/usr/bin/env python3
"""
MSI Survivor Lab v07 Boundary and Multi-Layer Exhaustion
====================
Tabbed GUI + headless verification tool + diagram and validation layer for the Modular Survivor Interface
...
- Known Theory Validation tab: Mertens/zeta, affine root-count, Goldbach local factor, polynomial root-count, observed-vs-predicted, and localization discrepancy checks.
- Pattern Search tab: localized complete-survivor nonemptiness scans, residue-class summaries, CSV export, and small-boundary exception detection.
- Layered Exhaustion tab: partial-period exhaustion by Q(y), residual-prime attrition bounds, and certification of localized nonemptiness.
- Boundary Completion tab/CLI: explicit small-prime boundary channels L_j=q, classification of interior vs boundary-only positives.
- Multi-layer residual grouping: optional staged reporting of residual attrition across levels y_0<...<z.

Run GUI:
    python msi_survivor_lab_v07_boundary_multilayer.py

Run tests:
    python msi_survivor_lab_v07_boundary_multilayer.py --self-test
...
if tk is not None:
    class SurvivorLabApp(tk.Tk):
        def __init__(self):
            super().__init__()
            self.title("MSI Survivor Lab v07 Boundary and Multi-Layer Exhaustion - Local, Global, and Windowed Envelopes")
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="MSI Survivor Lab v07 Boundary and Multi-Layer Exhaustion")
